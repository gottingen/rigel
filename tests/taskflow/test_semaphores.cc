...
void critical_section(size_t W) {
  ...
  rigel::CriticalSection section(1);

  int N = 1000;
  int counter = 0;

  for(int i=0; i<N; ++i) {
    rigel::Task task = taskflow.emplace([&](){ counter++; })
...
void overlapped_semaphore(size_t W) {
  ...
  rigel::Executor executor(W);
  rigel::Taskflow taskflow;
  rigel::Semaphore semaphore1(1);
  rigel::Semaphore semaphore4(4);
...
void conflict_graph(size_t W) {
  ...
  rigel::Executor executor(W);
  rigel::Taskflow taskflow;
  rigel::Semaphore conflict_AB(1);
  rigel::Semaphore conflict_AC(1);

  int counter {0};
  std::mutex mutex;

  rigel::Task A = taskflow.emplace([&](){ counter++; });

  // B and C can run together
  rigel::Task B = taskflow.emplace([&](){
