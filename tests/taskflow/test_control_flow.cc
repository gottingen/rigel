...
TEST_CASE("Cond.Types") {
  ...
  static_assert(rigel::is_condition_task_v<decltype(explicit_c)>);
  static_assert(rigel::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == rigel::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == rigel::TaskType::CONDITION);
}
...
void conditional_spawn(
  ...
  const int max_depth,
  int depth,
  rigel::Subflow& subflow
)  {
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      auto A = subflow.emplace([&](){ counter++; });
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](rigel::Subflow& subflow){
...
TEST_CASE("BTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    for(int l=1; l<12; l++) {
      rigel::Taskflow flow;
      std::vector<rigel::Task> prev_tasks;
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    rigel::Taskflow flow;
    flow.emplace([&](rigel::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  rigel::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (rigel::Subflow& subflow) mutable {
    //         ___________
    //        |           |
    //        v           |
    //   S -> A -> B -> cond
    ...
    auto B = subflow.emplace([&](rigel::Subflow& subflow){
...
void hierarchical_condition(unsigned w) {
  ...
  rigel::Executor executor(w);
  rigel::Taskflow tf0("c0");
  rigel::Taskflow tf1("c1");
  rigel::Taskflow tf2("c2");
  rigel::Taskflow tf3("top");
  ...
  auto sbf1 = tf3.emplace([&](rigel::Subflow& sbf){
...
void condition_subflow(unsigned W) {
  ...
  auto subflow = taskflow.emplace([&](rigel::Subflow& sf){
    sf.emplace([&, i](){
      REQUIRE(i<I);
      data[i] = i*(i+1)/2*123;;
...
TEST_CASE("MultiCond.Types") {
  ...
  auto explicit_mc = [](){
    rigel::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> rigel::SmallVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(rigel::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(rigel::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> rigel::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> rigel::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> rigel::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> rigel::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> rigel::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
