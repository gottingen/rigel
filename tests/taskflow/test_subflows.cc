...
void joined_subflow(unsigned W) {
  ...
  SUBCASE("Trivial") {
    ...
    rigel::Task subflow3, subflow3_;
    //std::future<int> fu3, fu3_;
    std::atomic<int> fu3v{0}, fu3v_{0};

    // empty flow
    auto subflow1 = tf.emplace([&] (rigel::Subflow& fb) {
      fu3v++;
      fb.join();
    }).name("subflow1");

    // nested empty flow
    auto subflow2 = tf.emplace([&] (rigel::Subflow& fb) {
      fu3v++;
      fb.emplace([&] (rigel::Subflow& fb) {
        fu3v++;
        fb.emplace( [&] (rigel::Subflow& fb) {
          fu3v++;
          fb.join();
    ...
    subflow3 = tf.emplace([&] (rigel::Subflow& fb) {
      ...
      subflow3_ = fb.emplace([&] (rigel::Subflow& fb) {
        REQUIRE(fu3v_ == 3);
        fu3v++;
        fu3v_++;
        //return 200;
  ...
  SUBCASE("Complex") {
    ...
    auto B = tf.emplace([&count, &data, &sum](rigel::Subflow& fb){
      ...
      auto n = fb.emplace([&count](rigel::Subflow& fb){
...
// --------------------------------------------------------
void detach_spawn(const int max_depth, std::atomic<int>& counter, int depth, rigel::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](rigel::Subflow& subflow){
...
void join_spawn(const int max_depth, std::atomic<int>& counter, int depth, rigel::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](rigel::Subflow& subflow){
...
void mix_spawn(
  const int max_depth, std::atomic<int>& counter, int depth, rigel::Subflow& subflow
) {
...
TEST_CASE("TreeSubflow" * doctest::timeout(300)) {
  ...
  SUBCASE("AllDetach") {
    ...
    for(int W=1; W<=4; W++) {
      ...
      rigel::Taskflow tf;
      tf.emplace([&](rigel::Subflow& subflow){
...
// Testcase: FibSubflow
// --------------------------------------------------------
int fibonacci_spawn(int n, rigel::Subflow& sbf) {
  if (n < 2) return n;
  int res1, res2;
  sbf.emplace([&res1, n] (rigel::Subflow& sbf) { res1 = fibonacci_spawn(n - 1, sbf); } );
  sbf.emplace([&res2, n] (rigel::Subflow& sbf) { res2 = fibonacci_spawn(n - 2, sbf); } );
...
void fibonacci(size_t W) {
  ...
  taskflow.emplace([&res, N] (rigel::Subflow& sbf) {
