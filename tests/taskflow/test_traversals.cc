...
TEST_CASE("DynamicTraversal" * doctest::timeout(300)) {
  ...
  std::function<void(Node*, rigel::Subflow&)> traverse;

  traverse = [&] (Node* n, rigel::Subflow& subflow) {
    REQUIRE(!n->visited);
    n->visited = true;
    ...
    for(size_t i=0; i<S; i++) {
      if(n->successors[i]->dependents.fetch_sub(1) == 1) {
        ...
        subflow.emplace([s=n->successors[i], &traverse](rigel::Subflow &subflow){
          traverse(s, subflow);
        });
      }
    }
  ...
  for(unsigned w=1; w<=4; w++) {
    ...
    for(size_t i=0; i<src.size(); i++) {
      tf.emplace([s=src[i], &traverse](rigel::Subflow& subflow){
        traverse(s, subflow);
      });
    }
...
/*void parallel_traversal(unsigned num_threads) {
  ...
  rigel::Executor executor(num_threads);
