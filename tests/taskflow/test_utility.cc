...
#include "rigel/taskflow/utility/traits.h"
#include "rigel/taskflow/utility/object_pool.h"
#include "rigel/taskflow/utility/small_vector.h"
#include "rigel/taskflow/utility/uuid.h"
#include "rigel/taskflow/utility/iterator.h"
#include "rigel/taskflow/utility/math.h"
...
TEST_CASE("SmallVector" * doctest::timeout(300)) {
  ...
  //SUBCASE("constructor")
  {
    rigel::SmallVector<int> vec1;
    REQUIRE(vec1.size() == 0);
    REQUIRE(vec1.empty() == true);

    rigel::SmallVector<int, 4> vec2;
  ...
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      rigel::SmallVector<int> vec1(N);
      for(auto& item : vec1) {
        item = N;
      }

      rigel::SmallVector<int> vec2(vec1);
      REQUIRE(vec1.size() == N);
      REQUIRE(vec2.size() == N);
  ...
  //SUBCASE("move_constructor")
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      ...
      rigel::SmallVector<int> vec2(std::move(vec1));
      REQUIRE(vec1.size() == 0);
      REQUIRE(vec1.empty() == true);
      REQUIRE(vec2.size() == N);
  ...
  for(int beg=-50; beg<=50; ++beg) {
    for(int end=-50; end<=50; ++end) {
      if(beg < end) {   // positive step
        for(int s=1; s<=50; s++) {
          REQUIRE((rigel::distance(beg, end, s) == count(beg, end, s)));
        }
      }
      else {            // negative step
        for(int s=-1; s>=-50; s--) {
...
void test_threaded_uuid(size_t N) {
  ...
  std::vector<rigel::UUID> uuids(65536);
...
TEST_CASE("ObjectPool.Sequential" * doctest::timeout(300)) {
  ...
  for(unsigned w=1; w<=4; w++) {
    ...
    rigel::ObjectPool<Poolable> pool(w);
...
TEST_CASE("RefWrapper" * doctest::timeout(300)) {
  ...
  static_assert(std::is_same<
    rigel::unwrap_reference_t<int>, int
  >::value, "");
  ...
  static_assert(std::is_same<
    rigel::unwrap_reference_t<std::reference_wrapper<int>>, int&
  >::value, "");

  static_assert(std::is_same<
    rigel::unwrap_reference_t<std::reference_wrapper<std::reference_wrapper<int>>>,
    std::reference_wrapper<int>&
  >::value, "");

  static_assert(std::is_same<
    rigel::unwrap_ref_decay_t<int>, int
  >::value, "");
  ...
  static_assert(std::is_same<
    rigel::unwrap_ref_decay_t<std::reference_wrapper<int>>, int&
  >::value, "");

  static_assert(std::is_same<
    rigel::unwrap_ref_decay_t<std::reference_wrapper<std::reference_wrapper<int>>>,
    std::reference_wrapper<int>&
  >::value, "");
...
//
//  SUBCASE("func1") {
//    using func1_traits = rigel::function_traits<decltype(func1)>;
//    static_assert(std::is_same<func1_traits::return_type, void>::value, "");
//    static_assert(func1_traits::arity == 0, "");
//  }
//
//  SUBCASE("func2") {
//    using func2_traits = rigel::function_traits<decltype(func2)>;
...
//      return 1;
//    };
//    using lambda1_traits = rigel::function_traits<decltype(lambda1)>;
...
//    };
//    using lambda2_traits = rigel::function_traits<decltype(lambda2)>;
...
//    };
//    using foo_traits = rigel::function_traits<foo>;
...
//    using ft1 = rigel::function_traits<std::function<void()>>;
//    static_assert(std::is_same<ft1::return_type, void>::value, "");
//    static_assert(ft1::arity == 0, "");
//
//    using ft2 = rigel::function_traits<std::function<int(int&, double&&)>&>;
...
//
//    using ft3 = rigel::function_traits<std::function<int(int&, double&&)>&&>;
...
//
//    using ft4 = rigel::function_traits<const std::function<void(int)>&>;
...
TEST_CASE("NextPow2") {
  ...
  static_assert(rigel::next_pow2(0u) == 1);
  static_assert(rigel::next_pow2(1u) == 1);
  static_assert(rigel::next_pow2(100u) == 128u);
  static_assert(rigel::next_pow2(245u) == 256u);
  static_assert(rigel::next_pow2(512u) == 512u);
  static_assert(rigel::next_pow2(513u) == 1024u);

  REQUIRE(rigel::next_pow2(0u) == 1u);
  REQUIRE(rigel::next_pow2(2u) == 2u);
  REQUIRE(rigel::next_pow2(1u) == 1u);
  REQUIRE(rigel::next_pow2(33u) == 64u);
  REQUIRE(rigel::next_pow2(100u) == 128u);
  REQUIRE(rigel::next_pow2(211u) == 256u);
  REQUIRE(rigel::next_pow2(23u) == 32u);
  REQUIRE(rigel::next_pow2(54u) == 64u);

  uint64_t z = 0;
  uint64_t a = 1;
  REQUIRE(rigel::next_pow2(z) == 1);
  REQUIRE(rigel::next_pow2(a) == a);
  REQUIRE(rigel::next_pow2((a<<5)  + 0) == (a<<5));
  REQUIRE(rigel::next_pow2((a<<5)  + 1) == (a<<6));
  REQUIRE(rigel::next_pow2((a<<32) + 0) == (a<<32));
  REQUIRE(rigel::next_pow2((a<<32) + 1) == (a<<33));
  REQUIRE(rigel::next_pow2((a<<41) + 0) == (a<<41));
  REQUIRE(rigel::next_pow2((a<<41) + 1) == (a<<42));

  REQUIRE(rigel::is_pow2(0) == false);
  REQUIRE(rigel::is_pow2(1) == true);
  REQUIRE(rigel::is_pow2(2) == true);
  REQUIRE(rigel::is_pow2(3) == false);
  REQUIRE(rigel::is_pow2(0u) == false);
  REQUIRE(rigel::is_pow2(1u) == true);
  REQUIRE(rigel::is_pow2(54u) == false);
  REQUIRE(rigel::is_pow2(64u) == true);
}
