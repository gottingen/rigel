...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  rigel::Taskflow taskflow, taskflow2;

  auto t1 = taskflow.emplace([](){});
  auto t2 = taskflow.emplace([](){ return 1; });
  auto t3 = taskflow.emplace([](rigel::Subflow&){ });
  auto t4 = taskflow.composed_of(taskflow2);
  auto t5 = taskflow.emplace([](){ return rigel::SmallVector{1, 2}; });
  auto t6 = taskflow.emplace([](rigel::Runtime&){});
  auto t7 = taskflow.emplace([](rigel::Runtime&){ return 1; });
  auto t8 = taskflow.emplace([](rigel::Runtime&){ return rigel::SmallVector{1, 2}; });

  REQUIRE(t1.type() == rigel::TaskType::STATIC);
  REQUIRE(t2.type() == rigel::TaskType::CONDITION);
  REQUIRE(t3.type() == rigel::TaskType::DYNAMIC);
  REQUIRE(t4.type() == rigel::TaskType::MODULE);
  REQUIRE(t5.type() == rigel::TaskType::CONDITION);
  REQUIRE(t6.type() == rigel::TaskType::STATIC);
  REQUIRE(t7.type() == rigel::TaskType::CONDITION);
  REQUIRE(t8.type() == rigel::TaskType::CONDITION);
}
...
TEST_CASE("Builder" * doctest::timeout(300)) {
  ...
  std::vector<rigel::Task> silent_tasks;
  std::vector<rigel::Task> tasks;
...
TEST_CASE("Iterators" * doctest::timeout(300)) {
  ...
  SUBCASE("Order") {
    ...
    A.for_each_successor([&, i=0] (rigel::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == B);
        break;
    ...
    E.for_each_dependent([&, i=0](rigel::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == A);
        break;
  ...
  SUBCASE("Generic") {
    ...
    taskflow.for_each_task([&tasks](rigel::Task s){
      tasks.push_back(s);
    });
    ...
    A.for_each_successor([B](rigel::Task s){ REQUIRE(s==B); });
    B.for_each_dependent([A](rigel::Task s){ REQUIRE(s==A); });
    ...
    A.for_each_successor([&](rigel::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
    ...
    counter = a = b = c = d = e = 0;
    B.for_each_dependent([&](rigel::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
...
void sequential_runs(unsigned W) {
  ...
  SUBCASE("RunWithFuture") {
    ...
    auto B = f.emplace([&](rigel::Subflow& subflow){
      count ++;
    ...
    std::list<rigel::Future<void>> fu_list;
  ...
  SUBCASE("MultipleRuns") {
    ...
    rigel::Taskflow tf1, tf2, tf3, tf4;
...
TEST_CASE("RunAndWait.Simple") {
  ...
  rigel::Taskflow others;
  rigel::Task A = others.emplace([&](){ counter++; });
  rigel::Task B = others.emplace([&](){ counter++; });
  A.precede(B);

  // main taskflow
  rigel::Task C = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 2);
  });
  rigel::Task D = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 4);
  });
  C.precede(D);
...
TEST_CASE("RunAndWait.Complex") {
  ...
  std::array<rigel::Taskflow, N> taskflows;
...
void worker_id(unsigned w) {
  ...
  for(int i=0; i<1000; i++) {
    ...
    auto B = taskflow.emplace([&](rigel::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id>=0);
      REQUIRE(id< w);
      sf.emplace([&](){
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (rigel::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    rigel::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<rigel::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<rigel::Taskflow>();
...
TEST_CASE("Reduce" * doctest::timeout(300)) {
  ...
  const auto plus_test = [](unsigned num_workers, auto &&data){
    rigel::Executor executor(num_workers);
    rigel::Taskflow tf;
    int result {0};
...
void observer(unsigned w) {
  ...
  auto observer = executor.make_observer<rigel::ChromeObserver>();

  rigel::Taskflow taskflowA;
  std::vector<rigel::Task> tasks;
  // Static tasking
  for(auto i=0; i < 64; i ++) {
