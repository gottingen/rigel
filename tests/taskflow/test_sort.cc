...
#include "rigel/taskflow/algorithm/sort.h"
...
TEST_CASE("BubbleSort" * doctest::timeout(300)) {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 1000; end += 200) {
      ...
      rigel::Taskflow taskflow("BubbleSort");
      ...
      auto even_phase = taskflow.emplace([&](rigel::Subflow& sf){
      ...
      auto odd_phase = taskflow.emplace([&](rigel::Subflow& sf) {
...
TEST_CASE("SelectionSort" * doctest::timeout(300)) {
  ...
  std::function<
    void(rigel::Subflow& sf, std::vector<int>&, int, int, int&)
  > spawn;
  ...
  spawn = [&] (
    ...
    auto SL = sf.emplace(
      [&spawn, &data, beg, m, l=minl] (rigel::Subflow& sf) mutable {
    ...
    auto SR = sf.emplace(
      [&spawn, &data, m, end, r=minr] (rigel::Subflow& sf) mutable {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 512; end <<= 1) {
      rigel::Taskflow taskflow("SelectionSort");
      ...
      auto argmin = taskflow.emplace(
        [&spawn, &data, &beg, end, &min](rigel::Subflow& sf) mutable {
...
TEST_CASE("MergeSort" * doctest::timeout(300)) {
  ...
  std::function<void(rigel::Subflow& sf, std::vector<int>&, int, int)> spawn;

  spawn = [&] (rigel::Subflow& sf, std::vector<int>& data, int beg, int end) mutable {
    ...
    auto SL = sf.emplace([&spawn, &data, beg, m] (rigel::Subflow& sf) {
      spawn(sf, data, beg, m);
    }).name(std::string("[")
          + std::to_string(beg)
          + ':'
    ...
    auto SR = sf.emplace([&spawn, &data, m, end] (rigel::Subflow& sf) {
      spawn(sf, data, m, end);
    }).name(std::string("[")
          + std::to_string(m)
          + ':'
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 10000; end = end * 10) {
      rigel::Taskflow taskflow("MergeSort");
      ...
      taskflow.emplace([&spawn, &data, end](rigel::Subflow& sf){
        spawn(sf, data, 0, end);
      }).name(std::string("[0")
            + ":"
            + std::to_string(end) + ")");
...
TEST_CASE("QuickSort" * doctest::timeout(300)) {
  ...
  std::function<void(rigel::Subflow& sf, std::vector<int>&, itr_t, itr_t)> spawn;
  ...
  spawn = [&] (
    ...
    sf.emplace([&spawn, &data, beg, pvt] (rigel::Subflow& sf) {
    ...
    sf.emplace([&spawn, &data, pvt, end] (rigel::Subflow& sf) {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 16384; end <<= 1) {
      ...
      rigel::Taskflow taskflow("QuickSort");
      ...
      taskflow.emplace([&spawn, &data](rigel::Subflow& sf){
