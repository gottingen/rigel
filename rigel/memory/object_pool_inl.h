...
#ifndef MELON_MEMORY_OBJECT_POOL_INL_H_
#define MELON_MEMORY_OBJECT_POOL_INL_H_
...
#include <vector>
#include <atomic>
#include "turbo/concurrent/scoped_lock.h"            // TURBO_SCOPED_LOCK
#include "rigel/thread/cleanup.h"           // MELON_THREAD_LOCAL
#include "turbo/platform/port.h"

#ifdef MELON_OBJECT_POOL_NEED_FREE_ITEM_NUM
...
#undef MELON_OBJECT_POOL_GET
        ...
        void clear_objects() {
            ...
            if (lp) {
                _local_pool = nullptr;
                CancelThreadExit(LocalPool::delete_local_pool, lp);
                delete lp;
            }
        }
    ...
    private:
        ...
        inline LocalPool *get_or_new_local_pool() {
            ...
            _local_pool = lp;
            CleanupThreadExit(LocalPool::delete_local_pool, lp);
...
#ifdef MELON_OBJECT_POOL_NEED_FREE_ITEM_NUM
    template <typename T>
    rigel::static_atomic<size_t> ObjectPool<T>::_global_nfree = MELON_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // MELON_MEMORY_OBJECT_POOL_INL_H_
