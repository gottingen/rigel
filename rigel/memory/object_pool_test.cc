...
#include "rigel/memory/object_pool.h"
...
namespace {
    ...
    void *get_and_return_int(void *) {
        ...
        printf("[%lu] warmup=%" PRId64 "\n", (size_t) pthread_self(), tm0.ElapsedNano());
        ...
        for (int j = 0; j < 5; ++j) {
            ...
            printf("[%lu:%d] get<D>=%.1f return<D>=%.1f\n",
                   (size_t) pthread_self(), j, tm1.ElapsedNano() / (double) N,
                   tm2.ElapsedNano() / (double) N);
        }
        return nullptr;
    }
