...
namespace rigel {
    ...
    namespace {
        std::string TURBO_MAYBE_UNUSED VectorToString(const std::vector<int> &vec) {
            std::string str("[");
        ...
        turbo::Status
        TURBO_MAYBE_UNUSED ParseIntOption(const std::string &msg, const std::string &arg, int *value) {
            try {
                *value = std::stoi(arg);
            }
...
// NUMA setting will be ignored on Windows platform
#if defined(TURBO_PLATFORM_APPLE) || defined(TURBO_PLATFORM_WINDOWS)

    turbo::Status
    SetNumaConfigOnThread(
            const std::map<std::string, std::string> &host_policy) {
        TURBO_UNUSED(host_policy);
        return turbo::OkStatus();
    }

    turbo::Status SetNumaMemoryPolicy(const std::map<std::string, std::string> &host_policy) {
        TURBO_UNUSED(host_policy);
        return turbo::OkStatus();
    }
    ...
    turbo::Status
    SetNumaMemoryPolicy(const std::map<std::string, std::string>& host_policy)
    {
