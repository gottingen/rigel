...
#include "rigel/thread/cleanup.h"
#include <vector>
...
namespace rigel::detail {
    ...
    public:
        ...
        turbo::Status add(Fn fn, void *arg) {
            try {
                if (_fns.capacity() < 16) {
                    _fns.reserve(16);
                }
                ...
                errno = ENOMEM;
                return turbo::ErrnoToStatus(ENOMEM, "");
            }
            return turbo::OkStatus();
        }

        void remove(Fn fn, void *arg) {
            auto it = std::find(_fns.begin(), _fns.end(), std::make_pair(fn, arg));
            if (it != _fns.end()) {
                auto ite = it + 1;
...
namespace rigel {
    ...
    turbo::Status CleanupThreadExit(void (*fn)(void *), void *arg) {
        if (nullptr == fn) {
            errno = EINVAL;
            return turbo::ErrnoToStatus(EINVAL, "");
        }
    ...
    turbo::Status CleanupThreadExit(void (*fn)()) {
        if (nullptr == fn) {
            errno = EINVAL;
            return turbo::ErrnoToStatus(EINVAL, "");
        }
        return CleanupThreadExit(detail::call_single_arg_fn, (void *) fn);
    }

    void CancelThreadExit(void (*fn)(void *), void *arg) {
        if (fn != nullptr) {
    ...
    void CancelThreadExit(void (*fn)()) {
        if (nullptr != fn) {
            CancelThreadExit(detail::call_single_arg_fn, (void *) fn);
        }
    }
