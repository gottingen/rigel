...
dependency between two tasks. A task is one of the following types:
  ...
  2. dynamic task        : the callable constructible from
                           @c std::function<void(rigel::Subflow&)>
  3. condition task      : the callable constructible from
                           @c std::function<int()>
  4. multi-condition task: the callable constructible from
                           @c %std::function<rigel::SmallVector<int>()>
  5. module task         : the task constructed from rigel::Taskflow::composed_of
                           @c std::function<void(rigel::Runtime&)>
...
rigel::Taskflow taskflow("simple");

rigel::Task A = taskflow.emplace([](){ std::cout << "TaskA\n"; });
rigel::Task B = taskflow.emplace([](){ std::cout << "TaskB\n"; });
rigel::Task C = taskflow.emplace([](){ std::cout << "TaskC\n"; });
rigel::Task D = taskflow.emplace([](){ std::cout << "TaskD\n"; });
...
class Taskflow : public FlowBuilder {
  ...
  public:
    ...
    @code{.cpp}
    rigel::Taskflow taskflow("My Taskflow");
    ...
    */
    ...
    @code{.cpp}
    rigel::Taskflow taskflow1(std::move(taskflow2));
    ...
    */
    ...
    @code{.cpp}
    rigel::Task parent = taskflow.emplace([](rigel::Subflow sf){
    ...
    */
    ...
    This method is similar to rigel::Taskflow::dump(std::ostream& ostream),
    ...
    */
    ...
    rigel::Taskflow::num_tasks is zero.
    */
    bool empty() const;
    ...
    */
    ...
    A visitor is a callable that takes an argument of type rigel::Task
    and returns nothing. The following example iterates each task in a
    taskflow and prints its name:

    @code{.cpp}
    taskflow.for_each_task([](rigel::Task task){
      std::cout << task.name() << '\n';
    });
    @endcode
    */
    ...
    A graph object (of type rigel::Graph) is the ultimate storage for the
...
rigel::Future is a derived class from std::future that will eventually hold the
execution result of a submitted taskflow (rigel::Executor::run)
or an asynchronous task (rigel::Executor::async, rigel::Executor::silent_async).
In addition to the base methods inherited from std::future,
you can call rigel::Future::cancel to cancel the execution of the running taskflow
...
// submit the taskflow
rigel::Future fu = executor.run(taskflow);
...
class Future : public std::future<T>  {
  ...
  public:
    ...
    */
    ...
    (non-preemptive).
    You can call rigel::Future::wait to wait for the cancellation to complete.
    */
    bool cancel();
