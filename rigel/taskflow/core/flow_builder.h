...
#include "rigel/taskflow/algorithm/partitioner.h"
...
namespace rigel {
    ...
    from which rigel::Taskflow and rigel::Subflow are derived.
    ...
    */
    ...
    public:
        ...
        */
        ...
        @return a rigel::Task handle

        The following example creates a static task.

        @code{.cpp}
        rigel::Task static_task = taskflow.emplace([](){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(rigel::Subflow&)>

        @param callable callable to construct a dynamic task

        @return a rigel::Task handle

        The following example creates a dynamic task (rigel::Subflow)
        that spawns two static tasks.

        @code{.cpp}
        rigel::Task dynamic_task = taskflow.emplace([](rigel::Subflow& sf){
          rigel::Task static_task1 = sf.emplace([](){});
          rigel::Task static_task2 = sf.emplace([](){});
        });
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from
                std::function<rigel::SmallVector<int>()>
        ...
        */
        ...
        @code{.cpp}
        rigel::Task A = taskflow.emplace([](){ std::cout << "A"; });
        rigel::Task B = taskflow.emplace([](){ std::cout << "B"; });
        rigel::Task C = taskflow.emplace([](){ std::cout << "C"; });
        rigel::Task D = taskflow.emplace([](){ std::cout << "D"; });
        A.precede(B, C, D);
        ...
        */
        ...
        @code{.cpp}
        rigel::Taskflow t1, t2;
        t1.emplace([](){ std::cout << "t1"; });

        // t2 is partially composed of t1
        rigel::Task comp = t2.composed_of(t1);
        rigel::Task init = t2.emplace([](){ std::cout << "t2"; });
        init.precede(comp);
        @endcode
        ...
        type rigel::Graph such that it can interact with the executor.
        For example:
        ...
        struct MyObj {
          rigel::Graph graph;
          MyObj() {
            rigel::FlowBuilder builder(graph);
            rigel::Task task = builder.emplace([](){
              std::cout << "a task\n";  // static task
            });
          }
        ...
        MyObj obj;
        rigel::Task comp = taskflow.composed_of(obj);
        @endcode
        ...
        */
        ...
        rigel::Task placeholder = taskflow.placeholder();
        ...
        */
        ...
        rigel::Task C = taskflow.emplace([](){ std::cout << "C"; });
        rigel::Task D = taskflow.emplace([](){ std::cout << "D"; });
        std::vector<rigel::Task> tasks {A, B, C, D}
        ...
        */
        ...
        @tparam P partitioner type (default rigel::GuidedPartitioner)
        ...
        */
        ...
        @param part partitioning algorithm (default rigel::GuidedPartitioner)
...
rigel::Subflow is a derived class from rigel::Runtime with a specialized mechanism
to manage the execution of a child graph.
By default, a subflow automatically @em joins its parent node.
You may explicitly join or detach a subflow by calling rigel::Subflow::join
or rigel::Subflow::detach, respectively.
...
rigel::Task A = taskflow.emplace([](){}).name("A");
rigel::Task C = taskflow.emplace([](){}).name("C");
rigel::Task D = taskflow.emplace([](){}).name("D");

// create a subflow graph (dynamic tasking)
rigel::Task B = taskflow.emplace([] (rigel::Subflow& subflow) {
  rigel::Task B1 = subflow.emplace([](){}).name("B1");
  rigel::Task B2 = subflow.emplace([](){}).name("B2");
  rigel::Task B3 = subflow.emplace([](){}).name("B3");
  B1.precede(B3);
  B2.precede(B3);
}).name("B");
