...
#include "rigel/taskflow/core/worker.h"
...
The rigel::ObserverInterface class allows users to define custom methods to monitor
...
rigel::ObserverInterface provides immutable wrappers,
rigel::WorkerView and rigel::TaskView, over workers and tasks.

Please refer to rigel::WorkerView and rigel::TaskView for details.

Example usage:

@code{.cpp}

struct MyObserver : public rigel::ObserverInterface {
  ...
  void on_entry(WorkerView w, rigel::TaskView tv) override final {
    std::ostringstream oss;
    oss << "worker " << w.id() << " ready to run " << tv.name() << '\n';
    std::cout << oss.str();
  }

  void on_exit(WorkerView w, rigel::TaskView tv) override final {
    std::ostringstream oss;
...
A rigel::ChromeObserver inherits rigel::ObserverInterface and defines methods to dump
...
// create a custom observer
std::shared_ptr<rigel::ChromeObserver> observer = executor.make_observer<rigel::ChromeObserver>();
...
A rigel::TFProfObserver inherits rigel::ObserverInterface and defines methods to dump
the observed thread activities into a format that can be visualized through
@TFProf.
...
// create a custom observer
std::shared_ptr<rigel::TFProfObserver> observer = executor.make_observer<rigel::TFProfObserver>();
