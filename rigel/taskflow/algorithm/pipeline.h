...
Pipeflow can only be created privately by the rigel::Pipeline and
...
@em parallel direction or @em serial direction (specified by rigel::PipeType)
and is coupled with a callable to invoke by the pipeline scheduler.
The callable must take a referenced rigel::Pipeflow object in the first argument:

@code{.cpp}
Pipe{PipeType::SERIAL, [](rigel::Pipeflow&){}}
@endcode
...
*/
    template<typename C = std::function<void(rigel::Pipeflow &)>>
    class Pipe {
    ...
    public:
        ...
        */
        ...
        @param d pipe type (rigel::PipeType)
        @param callable callable type

        The constructor constructs a pipe with the given direction
        (rigel::PipeType::SERIAL or rigel::PipeType::PARALLEL) and the given callable.
        The callable must take a referenced rigel::Pipeflow object in the first argument.
        ...
        */
        ...
        @param type a rigel::PipeType variable
        */
        ...
        @param callable a callable object constructible from std::function<void(rigel::Pipeflow&)>
...
rigel::Pipeline pipeline(num_lines,
...
rigel::Task task = taskflow.composed_of(pipeline)
                        .name("pipeline");
...
Internally, rigel::Pipeline uses std::tuple to store the given sequence of pipes.
...
If applications need to change these pipes, please use rigel::ScalablePipeline.
*/
    template<typename... Ps>
    class Pipeline {
...
<i>pipeline scheduling framework</i> using a module task in a taskflow.
Unlike rigel::Pipeline that instantiates all pipes upon the construction time,
rigel::ScalablePipeline allows variable assignments of pipes using range iterators.
...
rigel::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
...
Each pipe has the same type of `%rigel::Pipe<%std::function<void(%rigel::Pipeflow&)>>`
