...
namespace rigel {
    ...
    (specified by rigel::PipeType) and is associated with a callable to invoke
    by the pipeline scheduler.

    You need to use the template function, rigel::make_data_pipe, to create
    a data pipe. The input and output types of a rigel::DataPipe should be decayed types
    ...
    @code{.cpp}
    rigel::make_data_pipe<int, std::string>(
    ...
    of rigel::Pipeflow in the second argument to probe the runtime information
    for a stage task, such as its line number and token number:

    @code{.cpp}
    rigel::make_data_pipe<int, std::string>(
      rigel::PipeType::SERIAL,
      [](int& input, rigel::Pipeflow& pf) {
    ...
    */
    ...
    public:
        ...
        */
        ...
        You should use the helper function, rigel::make_data_pipe,
        to create a DataPipe object, especially when you need rigel::DataPipe
        ...
        */
        ...
        A data pipe can be either parallel (rigel::PipeType::PARALLEL) or serial
        (rigel::PipeType::SERIAL).
        */
        PipeType type() const {
            return _type;
...
/**
@brief function to construct a data pipe (rigel::DataPipe)

@tparam Input input data type
@tparam Output output data type
@tparam C callable type

rigel::make_data_pipe is a helper function to create a data pipe (rigel::DataPipe)
in a data-parallel pipeline (rigel::DataPipeline).
The first argument specifies the direction of the data pipe,
either rigel::PipeType::SERIAL or rigel::PipeType::PARALLEL,
...
The callable can additionally take a reference of rigel::Pipeflow,
...
Similar to rigel::Pipeline, a rigel::DataPipeline is a composable graph object
for users to create a <i>data-parallel pipeline scheduling framework</i> 
using a module task in a taskflow.
The only difference is that rigel::DataPipeline provides a data abstraction
...
int main() {
  ...
  rigel::DataPipeline pl(num_lines,
    rigel::make_data_pipe<void, int>(rigel::PipeType::SERIAL, [&](rigel::Pipeflow& pf) -> int{
      if(pf.token() == 5) {
        pf.stop();
        return 0;
      }
...
*/
    ...
    public:
        ...
        */
        ...
        The first pipe must define a serial direction (rigel::PipeType::SERIAL)
    ...
    void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, Runtime &) {
        ...
        visit_tuple([&](auto &&pipe) {
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                // [](rigel::Pipeflow&) -> void {}, i.e., we only have one pipe
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(pf);
                    // [](rigel::Pipeflow&) -> output_t {}
                } else {
                ...
                // [](input_t&, rigel::Pipeflow&) -> void {}
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
                    // [](input_t&, rigel::Pipeflow&) -> output_t {}
                } else {
                    _buffer[pf._line].data = pipe._callable(
...
// Procedure: _build
    ...
    void DataPipeline<Ps...>::_build() {
        ...
        for (size_t l = 0; l < num_lines(); l++) {
            ...
            _tasks[l + 1] = fb.emplace([this, l](rigel::Runtime &rt) mutable {
