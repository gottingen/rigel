...
int main() {
  ...
  const size_t num_lines = 4;
    ...
    //1. How can I put a placeholder in the first pipe, i.e. [] (void, rigel::Pipeflow&) in order to match the pipe vector?
    auto pipe_callable1 = [] (rigel::Pipeflow& pf) mutable -> int {
        if(pf.token() == 5) {
          pf.stop();
          return 0;
        }
    ...
    auto pipe_callable2 = [] (int input, rigel::Pipeflow& pf) mutable -> float {
        return input + 1.0;
    };
    auto pipe_callable3 = [] (float input, rigel::Pipeflow& pf) mutable -> int {
        return input + 1;
    };
  ...
  pipes.emplace_back(rigel::make_scalable_datapipe<void, int>(rigel::PipeType::SERIAL, pipe_callable1));
  pipes.emplace_back(rigel::make_scalable_datapipe<int, float>(rigel::PipeType::SERIAL, pipe_callable2));
  pipes.emplace_back(rigel::make_scalable_datapipe<float, int>(rigel::PipeType::SERIAL, pipe_callable3));
  ...
  pipes.emplace_back(rigel::make_scalable_datapipe<int, float>(rigel::PipeType::SERIAL, pipe_callable1));
  pipes.emplace_back(rigel::make_scalable_datapipe<float, int>(rigel::PipeType::SERIAL, pipe_callable1));
